  ...
  useEffect(() => {
    if (isPreparing) {
      ...
      if (vergangenerSpielzug?.koordinaten) {
        ...
        setCityCoordinates({ lat: parseFloat(lat), lng: parseFloat(lng) });

        // Berechne Entfernung, wenn Zielkoordinaten vorhanden sind
        if (selectedCoordinates.current) {
          calculateDistance(
            selectedCoordinates.current.lat,
            selectedCoordinates.current.lng,
            parseFloat(lat),
            parseFloat(lng)
          );
        }
